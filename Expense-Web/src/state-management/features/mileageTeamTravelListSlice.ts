...
export const mileageTeamTravelListSlice = createSlice({
  name: "mileageTeamTravelList",
  initialState,
  reducers: {
    setSearchTextFilter(state, action: PayloadAction<string>) {
      state.search = action.payload;
      if (state.page !== 1) state.search = action.payload;
    },
...
export const {
  ...
  setColumnFilters,
  resetFilters,
} = mileageTeamTravelListSlice.actions;

export default mileageTeamTravelListSlice.reducer;
